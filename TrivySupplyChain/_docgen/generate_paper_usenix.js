...
body([{ text: "Finally, we use the verified model as a ground-truth oracle for an automated search over defender policies, scoring each candidate by its model-checked compromise probability plus an operational-cost penalty " }, cite(20), { text: ". Maximising the score converges on complete credential rotation as the minimal-cost provably-safe policy, recovering the paper's central recommendation without human guidance. The search proposes; the model checker decides." }]);
